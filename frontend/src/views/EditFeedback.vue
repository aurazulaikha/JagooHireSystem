  ...
  <div class="dashboard-wrapper">
    <div class="dashboard-container">
      ...
      <div class="dashboard-header">
        <h1 class="dashboard-title">Edit Feedback</h1>
      ...
      <div v-else-if="error" class="error-state">
        <p>{{ error }}</p>
        <button @click="goBack" class="btn-secondary">Kembali ke List</button>
      </div>
      ...
      <div v-else class="feedback-content">
        ...
        <!-- Feedback Form -->
        <form @submit.prevent="updateFeedback" class="feedback-form">
          <!-- Rating -->
          ...
          <div class="form-actions">
            ...
            <button type="submit" :disabled="updating" class="btn-primary">
              {{ updating ? 'Memperbarui...' : '💾 Update Feedback' }}
            </button>
          </div>
        </form>
      </div>
...
export default {
  ...
  async mounted() {
    console.log('EditFeedback mounted with ID:', this.feedbackId);
    await this.loadFeedbackData();
  },
  methods: {
    async loadFeedbackData() {
      if (!this.feedbackId) {
        this.error = 'Feedback ID tidak ditemukan';
        this.loading = false;
        return;
      }

      try {
        console.log('Fetching feedback data for ID:', this.feedbackId);
        
        const res = await axios.get(
          `http://localhost:5000/feedbacks/${this.feedbackId}`,
          {
            headers: { Authorization: `Bearer ${this.token}` }
          }
        );
        
        console.log('Feedback data received:', res.data);
        
        if (res.data.success && res.data.data) {
          this.feedbackData = res.data.data;
          this.form.rating = this.feedbackData.rating;
          this.form.comment = this.feedbackData.comment || '';
        } else {
          throw new Error('Data feedback tidak valid');
        }
        ...
        if (error.response?.status === 404) {
          ...
          this.error = 'Gagal memuat data feedback: ' + (error.response?.data?.message || error.message);
        }
        
        // Auto redirect setelah 3 detik jika error
        setTimeout(() => {
          this.goBack();
        }, 3000);
      } finally {
    ...
    async updateFeedback() {
      ...
      try {
        ...
        this.showCustomAlert('Feedback berhasil diperbarui!');
        ...
        const message = error.response?.data?.message || 'Gagal memperbarui feedback';
...
<style scoped>
/* Gunakan style yang sama dengan AddFeedback.vue untuk konsistensi */
.dashboard-wrapper {
